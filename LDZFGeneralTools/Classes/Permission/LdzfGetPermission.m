...
+(void)getAddressBookPermission:(void (^)(BOOL))callBack
{
    ...
    if (status == CNAuthorizationStatusNotDetermined) {//未决定
    ...
    }else if (status == CNAuthorizationStatusAuthorized){//已经授权过
        dispatch_async(dispatch_get_main_queue(), ^{
            callBack(YES);
        });
    }else if (status == CNAuthorizationStatusDenied){//用户已经明确否认了这个应用程序访问
        dispatch_async(dispatch_get_main_queue(), ^{
            callBack(NO);
        });
    }else{
